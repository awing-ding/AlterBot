...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let toArchive;
        if (!archivistRole) {
            return await interaction.reply({content: "Le rôle d'archiviste (staff wiki) n'est pas configuré !\nConfigurez le avec `/configure set roleArchivist <role>`", ephemeral: true});
        }
        if (interaction.options.getSubcommand() === "salon") {
            ...
            if (!archiveCategory) {
                return await interaction.reply({content: "La catégorie d'archive par défaut n'est pas configurée !\nConfigurez-la avec `/configure set categoryArchive <catégorie>`", ephemeral: true});
            }
            ...
            await category.permissionOverwrites.set([{
               id: interaction.guild.role.everyone.id,
                allow: [],
                deny: ['ViewChannel']
            },{
                id: archivistRole,
