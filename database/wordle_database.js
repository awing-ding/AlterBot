...
const wordleDatabase = function(){
    ...
    this.getPreviousGame = async function(bodyObject){
        return new Promise((resolve, reject) => {
    ...
    this.newGame = async function(bodyObject){
        /// word, date, score
        const {word, date, score} = JSON.parse(bodyObject)
        if (!word || !date || score === undefined){return Promise.reject("Missing parameters")}
        return new Promise((resolve, reject) => {
    ...
    this.newPlayer = async function(bodyObject){
        const {id} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.getGuessAndGame = async function(){
        const {playerId, gameStamp} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
            db.run(`
    ...
    this.getPlayer = async function(bodyObject){
        const {id} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.getPlayerGuesses = async function(bodyObject){
        const {id} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.updatePlayerStats = async function(bodyObject){
        const {playerId, stats} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.getPlayerActualGame = async function(bodyObject){
        const {playerId, gameStamp} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.addPlayerToGame = async function(bodyObject){
        const {playerId, gameStamp} = JSON.parse(bodyObject)
        return new Promise((resolve, reject) => {
    ...
    this.insertNewGuess = async function(bodyObject){
        let {playerId, gameStamp, guess, guessNumber, score} = JSON.parse(bodyObject)
        score = score ?? null
        const map = {
            0: 'first_guess',
            1: 'second_guess',
    ...
    this.getAllPlayers = async function(bodyObject){
        let {limit, page} = JSON.parse(bodyObject)
        limit = limit ?? 10
        page = page ?? 0
        return new Promise((resolve, reject) => {
            db.all("SELECT id, score_total FROM players ORDER BY score_total DESC LIMIT ? OFFSET ?", [limit, page * limit], (err, rows) => {
                if (err) {
    ...
    this.routes = {
        "GET" : {
            "allPlayers" : {
                params: ["limit?", "page?"],
                func: this.getAllPlayers

            },
            "playerGuesses" : {
                params: ["playerId"],
                func: this.getPlayerGuesses
            },
            "player" : {
                params: ["playerId"],
                func: this.getPlayer
            ...
            "playerActualGame" : {
                params: ["playerId", "gameStamp"],
                func : this.getPlayerActualGame
            },
            "currentGame" : {
                params : ["playerId", "gameStamp"],
                func: this.getCurrentGame,
            },
            "previousGame" : {
                params: [],
                func: this.getPreviousGame,
            },
            "guessAndGame" : {
                params : ["playerId", "gameStamp"],
                func: this.getGuessAndGame,
            },
        },
        "PUT" : {
            "newGame" : {
                params: ["word", "date", "score"],
                func: this.newGame
            },
            "newPlayer" : {
                params: ["id"],
            ...
            "updatePlayerStat" : {
                params: ["playerId", "stats"],
                func: this.updatePlayerStats
            },
            "addPlayerToGame" : {
                params: ["playerId", "gameStamp"],
                func: this.addPlayerToGame
            },
            "insertNewGuess" : {
                params: ["playerId", "gameStamp", "guess", "guessNumber", "score?"],
                func: this.insertNewGuess
            },
        }
    }
}
